...
function get_visitor_country($owner = "", $timeafter = 0) {
    global $CONFIG;
    ...
    $query = "SELECT  m.string, COUNT(*) as count, m.id from {$CONFIG->dbprefix}metastrings m JOIN {$CONFIG->dbprefix}system_log s ON m.id = s.object_id ";
    foreach ($where as $w) {
        $query .= " and $w";
    }

    $query .= " GROUP BY m.string";

    return get_data($query);

}

function get_most_comment_blog_by_user($owner = "", $timeafter = 0) {
    global $CONFIG;
...
function get_friends_log($performed_by_guid = "", $timeafter = 0) {
    global $CONFIG;
...
/**
 * log the visitor when user visit other users';
 */
function visitor_log() {
    global $CONFIG;
    static $visitorLogcache;
    ...
    {
        if (!is_array($visitorLogcache)) {
            $visitorLogcache = array();
        }
        ...
        // Has loggable interface, extract the necessary information and store
        $object_id = $value;
        $object_class = "ElggMetaStrings";
        $object_type = "";
        $object_subtype = "";
        $event = "Visitor";
        $time = time();
        $performed_by = $VisitorGuid;
        $access_id = ACCESS_LOGGED_IN;
        $enabled = 'yes';
        $owner_guid = $ProfileGuid;

        insert_data("INSERT DELAYED into {$CONFIG->dbprefix}system_log (object_id, object_class, object_type, object_subtype, event, performed_by_guid, owner_guid, access_id, enabled, time_created) VALUES ('$object_id','$object_class','$object_type', '$object_subtype', '$event',$performed_by, $owner_guid, $access_id, '$enabled', '$time')");

//
//        if ($time - $visitorLogcache[$owner_guid][$performed_by] > 5) {
//
//
//            $visitorLogcache[$owner_guid][$performed_by] = $time;
//        }
